## 高级软件工程报告作业  
该项目基于<a href="https://github.com/lucidrains/denoising-diffusion-pytorch.git">denoising-diffusion-pytorch</a>项目的基础上做了以下改动：   
1、将denoising_diffusion_pytorch.py按照工程化的思想分解成了四个独立的模块，每个模块分别负责去噪扩散模型的一个明确的功能。四个模块分别为GaussianDiffusion.py、Unet.py、trainer.py以及utils.py。  
具体来说，GaussianDiffusion.py负责实现扩散模型的相关逻辑和算法，其中包括对图像进行扩散过程的计算以及生成扩散步骤中的噪声等；  
Unet.py负责定义U-Net模型的网络结构，其中包括用于帮助提取图像特征的编码器-解码器架构的定义等等；  
...
数据集放于denoising-diffusion-pytorch/denoising_diffusion_pytorch/data文件夹中。跑了200000步的实验结果图放于denoising-diffusion-pytorch/denoising_diffusion_pytorch/results文件夹中  
